...
final class DocumentModel: ObservableObject {
    ...
    func midiInterfaceOptions() -> [MIDIInterfaceOption] {
        ...
        return midiSources.flatMap { source in
            let matchingDestinations = destinationsByName[source.displayName] ?? []
            return matchingDestinations.map { destination in
                MIDIInterfaceOption(
    ...
    private var dx100DeviceCacheSummaryRows: [KeyValueRow] {
        let fetchableBanks = selectedDeviceVoiceBanks
        return [
            ...
            KeyValueRow("Voice Banks", "\(cachedDX100VoiceBanks.count)/\(fetchableBanks.count)"),
    ...
    func refreshDX100DeviceCache(showsFailureAlert: Bool = false) {
        ...
        let sourceName = selectedSourceName(for: .dx100)
        let destinationName = selectedDestinationName(for: .dx100)
        let systemChannel = systemChannel
        isFetchingFromDevice = true
        deviceCacheStatus = "Fetching DX100 device cache..."
        statusMessage = "Fetching DX100 Internal bank from \(sourceName) -> \(destinationName)..."
        errorMessage = nil
        let progressPanel = EditorProgressPanel(
            title: "Fetching DX100 Device Cache",
            message: "The DX100 Internal bank is being cached. Please wait.",
            showsCancelButton: true
        )
        ...
        operationTask = Task {
            var selectionToOpen: DeviceVoiceBankWindowSelection?
            do {
                ...
                selectionToOpen = DeviceVoiceBankWindowSelection(device: .dx100, bank: 1)
                deviceCacheStatus = "Loaded 1 item"
                statusMessage = "DX100 Internal bank loaded from \(sourceName) -> \(destinationName)."
                errorMessage = nil
                try Task.checkCancellation()
                progressPanel.update(
                ...
                statusMessage = "DX100 Internal bank fetch cancelled."
                errorMessage = nil
            } catch {
                do {
                    ...
                    statusMessage = "DX100 current voice responds on \(sourceName) -> \(destinationName), but the Internal bank could not be fetched."
            ...
            progressPanel.dismiss()
            isFetchingFromDevice = false
            if let selectionToOpen {
                requestVoiceBankWindowOpen(selection: selectionToOpen)
            }
        }
    }
    ...
    func beginDX100ManualInternalCaptureFromSelection() {
        ...
        let destinationName = selectedDestinationName(for: .dx100)
        isFetchingFromDevice = true
        deviceCacheStatus = "Waiting for manual DX100 Internal bank capture..."
        statusMessage = "Waiting for a manual DX100 Internal bank dump from \(sourceName) -> \(destinationName)..."
        errorMessage = nil
    ...
    func cachedVoiceFetchResult(
        ...
        case let .dx100Bank(bank, voiceNumber):
            guard let voices = cachedDX100VoiceBanks[bank],
                  voices.indices.contains(voiceNumber) else {
                return nil
            }
            let dxVoice = voices[voiceNumber]
            guard let translated = try? dxVoice.fb01EditableVoice() else {
                return nil
            }
    ...
    func dx100VoiceBankSelectorItems(bank: Int, device: EditorDeviceSelection = .dx100) -> [VoiceBankSelectorItem] {
        ...
        let manualCaptureSlots = cachedDX100ManualCaptureSlots[bank] ?? []
        return voices.enumerated().map { index, voice in
            ...
            return VoiceBankSelectorItem(
                ...
                annotation: manualCaptureSlots.contains(index) ? "Manual" : nil
            )
        }
    }
    ...
    func dx100VoiceBankCaptureNote(bank: Int) -> String? {
        guard let kind = DX100ModuleServices.shared.module.voiceBankKind(displayBank: bank),
              ...
              cachedDX100VoiceBanks[bank] != nil else {
            return nil
        }

        let manualCount = (cachedDX100ManualCaptureSlots[bank] ?? []).count
        if manualCount > 0 {
            return "Assisted capture bank. \(manualCount) slot\(manualCount == 1 ? "" : "s") required explicit front-panel confirmation. Verify critical voices if the DX front panel may have changed during capture."
        }

        return "Assisted capture bank. Forest built this bank from current-voice dumps while stepping through the DX front panel. Verify critical voices if the DX front panel may have changed during capture."
    }
    ...
    private func fetchDX100SelectorCache(bank: Int) async {
        ...
        operationTask = Task {
            ...
            do {
                try Task.checkCancellation()
                guard let kind = DX100ModuleServices.shared.module.voiceBankKind(displayBank: bank) else {
                    throw FB01MIDIError.timedOut("DX100 voice bank")
                }
                ...
                if kind == .internalRAM {
                    ...
                    var manualCaptureSlots = Set<Int>()
                    ...
                    for voiceIndex in 0..<DX100ModuleServices.shared.module.voicesPerBank {
                        ...
                        fetchedVoices.append(fetchedVoice.voice)
                        if fetchedVoice.usedManualConfirmation {
                            manualCaptureSlots.insert(voiceIndex)
                        }
                    }

                    voices = fetchedVoices
                    cacheDX100VoiceBank(voices, bank: bank, manualCaptureSlots: manualCaptureSlots)
                } else {
                    var fetchedVoices: [DX100VoiceData] = []
                    fetchedVoices.reserveCapacity(DX100ModuleServices.shared.module.voicesPerBank)
                    for voiceIndex in 0..<DX100ModuleServices.shared.module.voicesPerBank {
                        ...
                        let fetched = try await Task.detached(priority: .userInitiated) {
                            try EditorVoiceDocumentService.fetchDX100DeviceVoice(
                                bank: bank,
                        ...
                        fetchedVoices.append(fetched.voice)
                    }
                    voices = fetchedVoices
                }
                ...
                if kind == .internalRAM {
                    ...
                    if kind.requiresManualBulkCapture {
                        selectionToOpen = DeviceVoiceBankWindowSelection(device: .dx100, bank: bank)
                    }
                }
    ...
    private func dx100SysExTroubleshootingMessage() -> String {
        """
        Forest can send notes to the DX100, but it did not receive a SysEx dump response.
    ...
    private func dx100ManualInternalDumpInstructions() -> String {
        """
        Automatic DX100 Internal-bank fetch did not reply over this MIDI transport.

        Forest can still capture the Internal bank if you trigger the dump from the DX100 front panel now:
        • press FUNCTION
        • select 5: SYS INFO
        • confirm SYS INFO = ON
    ...
    private func shouldOfferDX100ManualInternalDumpFallback(
        sourceIndex _: Int,
        destinationIndex _: Int
    ) -> Bool {
        // If the DX100 can receive notes but automatic SysEx bank capture times out,
        // a front-panel manual dump is still a useful recovery path on either wired
        // or Bluetooth transports.
        true
    }

    private func confirmDX100ManualInternalDumpCapture(contextTitle: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = "Manual DX100 \(contextTitle) Capture"
    ...
    private func fetchDX100InternalBankWithBluetoothFallback(
        ...
        do {
            ...
            progressPanel.update(
                message: "The DX100 \(contextTitle) is being fetched. Please wait.\nListening for a manual Internal bank dump...",
                completed: 0,
                total: 1
            )
    ...
    private func dx100VoiceBankResponseDiagnostic(_ bytes: [UInt8]) -> String? {
        ...
        return """
        DX100 replied with a Yamaha 32-voice bank-style SysEx message, but it was \(bytes.count) bytes instead of the expected \(expectedCount).
        ...
        For full DX100 bank capture, prefer a wired MIDI interface. Live editing and current-voice fetch can still work over the same transport.
        """
    }
    ...
    private func fetchDX100AssistedBankVoice(
        ...
        bankTitle: String,
        progressPanel: EditorProgressPanel
    ) async throws -> (voice: DX100VoiceData, usedManualConfirmation: Bool) {
        let automaticAttempts = 2
        var lastError: Error?
        let totalVoices = Double(DX100ModuleServices.shared.module.voicesPerBank)
        ...
        progressPanel.update(
            title: "Fetch Attempt 3",
            message: "Press Voice \(voiceNumber) on the front panel and click Continue.",
            completed: Double(voiceIndex),
            total: totalVoices
        )
        ...
        do {
            ...
            return (fetched.voice, true)
        } catch {
            lastError = error
        }

        throw lastError ?? FB01MIDIError.timedOut("\(bankTitle) voice \(voiceNumber)")
    }
    ...
    func replaceCachedDX100Voice(inBank bank: Int, slotIndex: Int, with voice: DX100VoiceData) throws {
        ...
        if var manualCaptureSlots = cachedDX100ManualCaptureSlots[bank], manualCaptureSlots.contains(slotIndex) {
            manualCaptureSlots.remove(slotIndex)
            cachedDX100ManualCaptureSlots[bank] = manualCaptureSlots
        }
    ...
    func swapCachedDX100Voices(inBank bank: Int, slotIndex: Int, with otherSlotIndex: Int) throws {
        ...
        if var manualCaptureSlots = cachedDX100ManualCaptureSlots[bank] {
            let firstWasManual = manualCaptureSlots.contains(slotIndex)
            let secondWasManual = manualCaptureSlots.contains(otherSlotIndex)
            if firstWasManual != secondWasManual {
                if firstWasManual {
                    manualCaptureSlots.remove(slotIndex)
                    manualCaptureSlots.insert(otherSlotIndex)
                } else {
                    manualCaptureSlots.remove(otherSlotIndex)
                    manualCaptureSlots.insert(slotIndex)
                }
                cachedDX100ManualCaptureSlots[bank] = manualCaptureSlots
            }
        }
    ...
    func cacheDX100VoiceBank(
        _ voices: [DX100VoiceData],
        bank: Int,
        rawBank: DX100VoiceBankData? = nil,
        manualCaptureSlots: Set<Int> = []
    ) {
        cachedDX100VoiceBanks[bank] = voices
        if let rawBank {
            cachedDX100RawVoiceBanks[bank] = rawBank
    ...
    func saveDX100VoiceBankFile(_ selector: EditorDocumentWorkspace.DX100VoiceBankFileSelector) {
        ...
        panel.message = "Save the displayed DX100 bank file as a voice bank file."
        panel.prompt = "Save Bank to File"
    ...
    func receiveExternalKeyboardMessage(_ message: [UInt8]) {
        ...
        if shouldSuppressExternalKeyboardEchoBackToDevice(for: message) {
            if isNoteOn, message.count > 1 {
                externalKeyboardStatus = "DX100 local keyboard direct; not echoed back."
            } else if isVolumeControl, message.count > 2 {
                externalKeyboardStatus = "DX100 local control \(message[1]) = \(message[2]); not echoed back."
            }
            updateExternalKeyboardPressedNotes(from: message)
            return
        }
    ...
    func sendLiveKeyboardReset() {
        guard !isBusy else {
            statusMessage = "Reset paused while the current device operation finishes."
            return
        }
        ...
        Task(priority: .high) { [weak self] in
            do {
                let press = try DX100.switchModeMessage(channel: channel, switchNumber: 27, value: 127)
                let release = try DX100.switchModeMessage(channel: channel, switchNumber: 27, value: 0)

                try await LiveMIDIPlaybackController.shared.sendImmediate(press, destinationIndex: destinationIndex)
                try await Task.sleep(nanoseconds: 100_000_000)
                try await LiveMIDIPlaybackController.shared.sendImmediate(release, destinationIndex: destinationIndex)

                await MainActor.run {
                    self?.liveKeyboardDisplay.status = "DX100 reset sent"
                    self?.externalKeyboardStatus = "DX100 reset sent to \(destinationName)"
                    self?.statusMessage = "Sent DX100 reset/play command to \(destinationName)."
                    self?.errorMessage = nil
                }
            } catch {
                await MainActor.run {
                    self?.errorMessage = "DX100 reset failed: \(error)"
                    self?.statusMessage = nil
                }
            }
        }
